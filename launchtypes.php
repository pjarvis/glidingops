...
$con=mysqli_connect($con_params['hostname'],$con_params['username'],$con_params['password'],$con_params['dbname']);
   ...
   {
    $q = "SELECT * FROM launchtypes WHERE id = " . $recid ;
    $r = mysqli_query($con,$q);
    $row = mysqli_fetch_array($r);
    $id_f = $row['id'];
    $name_f = htmlspecialchars($row['name'],ENT_QUOTES);
    $acronym_f = htmlspecialchars($row['acronym'],ENT_QUOTES);
    $trantype="Update";
    mysqli_close($con);
   }
  }
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
 ...
 $acronym_f = InputChecker($_POST["acronym_i"]);
 if (empty($acronym_f) )
 {
  $acronym_err = "ACRONYM is required";
  $error = 1;
 }
 if ($error != 1)
 {
...
$con=mysqli_connect($con_params['hostname'],$con_params['username'],$con_params['password'],$con_params['dbname']);
   ...
   else
   {
     ...
     if (isset($_POST["del"]) ) {if ($_POST["del"] == "Delete"){
       $Q="DELETE FROM launchtypes WHERE id = " . $_POST['updateid'] ;}}
     if (isset($_POST["tran"]) ) {if ($_POST["tran"] == "Update"){
      $Q = "UPDATE launchtypes SET ";
      $Q .= "name=";
      $Q .= "'" . mysqli_real_escape_string($con, $name_f)  . "'";
      $Q .= ",acronym=";
      $Q .= "'" . mysqli_real_escape_string($con, $acronym_f)  . "'";
$Q .= " WHERE ";$Q .= "id ";$Q .= "= ";
$Q .= $_POST['updateid'];}
     else
     if ($_POST["tran"] == "Create"){
       $Q = "INSERT INTO launchtypes (";$Q .= "name";$Q .= ", acronym";$Q .= " ) VALUES (";
       $Q .= "'" . mysqli_real_escape_string($con, $name_f) . "'";
       $Q.= ",";
       $Q .= "'" . mysqli_real_escape_string($con, $acronym_f) . "'";
      $Q.= ")";
    ...
    if ($_POST["tran"] == "Delete")
     header('Location: LaunchTypes');
    if ($_POST["tran"] == "Update")
     header('Location: LaunchTypes');
  }
 }
$name_f=htmlspecialchars($name_f,ENT_QUOTES);
$acronym_f=htmlspecialchars($acronym_f,ENT_QUOTES);
}
?>
<div id='divform'>
...
echo "<td>";
echo "<input ";
if (strlen($name_err) > 0) echo "class='err' ";echo "type='text' ";echo "name='name_i' ";echo "Value='";echo $name_f;echo "' ";echo "maxlength='40'";echo " autofocus ";echo ">";echo "</td>";echo "<td>";
echo $name_err; echo "</td></tr>";
}
?>
<?php if (true)
{
echo "<tr><td class='desc'>ACRONYM</td><td>*</td>";
echo "<td>";
echo "<input ";
if (strlen($acronym_err) > 0) echo "class='err' ";echo "type='text' ";echo "name='acronym_i' ";echo "Value='";echo $acronym_f;echo "' ";echo "maxlength='1'";echo ">";echo "</td>";echo "<td>";
echo $acronym_err; echo "</td></tr>";
}
?>
</table>
<table>
